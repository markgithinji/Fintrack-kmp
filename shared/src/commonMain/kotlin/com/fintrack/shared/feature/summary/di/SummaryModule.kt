...
val summaryModule = module {
    single { SummaryApi(client = get(), baseUrl = getProperty("baseUrl")) }
    single<SummaryRepository> { SummaryRepositoryImpl(api = get()) }
    viewModel { StatisticsViewModel(repo = get(), transactionRepo = get()) }
}